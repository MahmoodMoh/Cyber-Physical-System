...
void prepare_state_packet(void) {
	// Обновляем sensor_data перед упаковкой
	...
	State_Packet[4] = sensor_data.led_status; // Led status
	State_Packet[5] = sensor_data.checksum;
	State_Packet[6] = 0xAA; // End Marker
}

/* ===== SPI slave config ===== */

volatile uint8_t next_reply = 0x00;
volatile uint8_t command_received = 0; // Флаг для main loop (если обработка долгая)


void SPI_SlaveInit(void) {
	// Настройка направлений (MISO - выход, остальные - вход)
	...
	SPDR0 = 0; // Очистка буфера
	//uart_send_string("SPI_SlaveInit done \r\n");
}

ISR(SPI0_STC_vect) { // Используем вектор SPI0 для 328PB
	uint8_t input = SPDR0; // Читаем то, что прислал Мастер
	//uart_send_string("Got messege from master! \r\n");
	
	if (sending_state == 1) {
		// Мы ожидаем DUMMY-байты от Мастера, чтобы отдать ему пакет
		
		// Мастер только что получил State_Packet[packet_index]
		packet_index++;
		
		if (packet_index < STATE_PACKET_SIZE) {
			// Загружаем СЛЕДУЮЩИЙ байт пакета для следующей транзакции
			SPDR0 = State_Packet[packet_index];
			} else {
			// Весь пакет отправлен. Сбрасываем состояние.
			sending_state = 0;
			packet_index = 0;
			SPDR0 = 0x00; // Возвращаемся к дефолтному ответу
		}
		
		return; // Выходим из ISR, не обрабатывая input как новую команду
	}
	
	switch(input) {
		case CMD_LED_ON:
			PORTC |= (1 << PC1); // Например, включить LED (на PB5 встроенный LED на Uno, на PB проверьте пин)
			next_reply = 0x4F;   // Подготовим ответ "ОК" для СЛЕДУЮЩЕГО байта
		...
		case CMD_GET_STATE:
			// 1. Собираем свежие данные в State_Packet
			prepare_state_packet();
			
			// 2. Инициализируем передачу: ставим индекс на 0
			packet_index = 0;
			
			// 3. Загружаем ПЕРВЫЙ байт пакета (отправится при следующем DUMMY-байте)
			SPDR0 = State_Packet[packet_index];
			
			// 4. Устанавливаем флаг, что мы теперь в режиме отправки пакета
			sending_state = 1;
			//uart_send_string("CMD_GET_STATE: Starting multi-byte transfer. \r\n");
			break;

		case CMD_GET_SENSOR:
			// Подготовим данные сенсора для отправки
			// Мастер получит их, только когда пошлет следующий байт (Dummy)
			sensor_data.light_level = read_light_sensor();
		...
		case CMD_DUMMY:
			// Мастер просто качает данные, ничего менять не надо
			// Но можно подготовить дефолтное значение
			//uart_send_string("CMD_DUMMY \r\n");
			next_reply = 0x00;
			break;

		default:
			next_reply = 0xEE; // Код ошибки
			uart_send_string("SPI error: \r\n");
	}

	// Загружаем ответ в регистр.
	// ОН УЙДЕТ ТОЛЬКО ПРИ СЛЕДУЮЩЕЙ ТРАНЗАКЦИИ ОТ МАСТЕРА!
	SPDR0 = next_reply;
...
int main(void) {
    ...
    while (1) {
        ...
        if (wake_flag) {
			...
			SPI_SlaveInit();	// SPI init for connection check
